...
class TagsParameter(object):
    ...
    Decides whether a test should be picked up based on the test, class, and module tags.
    """
    ...
    def should_pick_up(self, tags):
        ...
        for tag in prep_tags:
            ...
            # If current env is excluded (eg. 'not reltest'), don't run the test
            # If current browser is excluded (eg. 'not ie8'), don't run the test
        ...
        for tag in prep_tags:
            ...
            for param_tag in self._parameter_tags:
                # If either the parameter tag or test tag is set to False (eg. 'not sierra'), don't run the test
                if param_tag.name == tag.name and (not param_tag.is_positive or not tag.is_positive):
                    return False
                # If the user has filtered tags using + (ie. sierra+snap), only run test that corresponds to both tags
                # The 'and not should_run' needs some explaining:
                # It's a fix for when you have more than one set of +-combinations
                # ie. tag1+tag2, tag3+tag4
                # if tag1+tag2 sets should_run to True, we've already determined that
                # the test should be run, hence we can skip the other combinations,
                # but we don't want to skip the entire function, in case there are any 'not' tags.
        ...
        # If none of the above checks caught anything, the <should_run> determines if the test gets picked up
        if self._parameter_tags:
            return should_run

        # Or if no tags where specified as parameters (eg. -t) we run the test (this will run ALL tests)
        return True
...
class TagsCollection(object):
    ...
    def prep_tags(tags):
        ...
        # 'not ' tags (is_positive = False) comes before positive tags. because False < True
...
class Tag(object):
    ...
    def __str__(self):
        return 'not {}'.format(self.name if not self.is_positive else self.name)

    def __eq__(self, other):
        if isinstance(other, Tag):
            return self.name == other.name and self.is_positive == other.is_positive
        else:
            return False
